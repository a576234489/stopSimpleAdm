...
<head>
  ...
  <script type="text/javascript" src="../../resources/js/customer/packinglot/packingLotService.js"></script>
  <script type="text/javascript" src="../../resources/js/customer/packinglot/packingLotController.js"></script>
</head>
<style>
	.label-a{
		width:70px
...
<body ng-app="stopSimple" ng-controller="packingLotController" ng-init="">
  ...
  }).use(['index', 'packingLot', 'table'], function(){
    ...
    var $ = layui.$, active = {
      ...
      add: function(){
        layer.open({
          type: 2
          ,title: '添加停车场 '
          ,content: '../../view/app/packinglot/listform.html'
          ,maxmin: true
          ,area: ["670px", "565px"]
          ,btn: ['确定', '取消']
          ,yes: function(index, layero){
        	  ...
        	  entity.address = othis.find('input[name="address"]').val();
        	  entity.longitude  = othis.find('input[name="longitude"]').val();
        	  entity.latitude  = othis.find('input[name="latitude"]').val();
        	  entity.province  = othis.find('#provice option:selected').val();
        	  entity.city  = othis.find('#city option:selected').val();
        	  entity.area  = othis.find('#area option:selected').val();
        	  entity.totalParkNumber  = othis.find('input[name="totalParkNumber"]').val();
        	  entity.openTime  = othis.find('input[name="openTime"]').val();
        	  entity.endTime  = othis.find('input[name="endTime"]').val();
        	  entity.appointPrice  = othis.find('input[name="appointPrice"]').val();
        	  entity.price  = othis.find('input[name="price"]').val();
        	  entity.remark  = othis.find('input[name="remark"]').val();
        	  entity.parkingLotTypeId  = othis.find('input[name="parkingLotTypeId"]').val();
        	  entity.paymentWay  = othis.find('input[name="paymentWay"]').val();
        	  entity.isLongrent  = othis.find('input[name="isLongrent"]').val();
        	  entity.isChargingpile  = othis.find('input[name="isChargingpile"]').val();
        	  entity.totalChargingPileNumber  = othis.find('input[name="totalChargingPileNumber"]').val();
