...
<body ng-app="stopSimple" ng-controller="roleController" ng-init="">
      ...
      <form class="layui-form">
		<table cellpadding="0" cellspacing="0" class="screenBox">
			<tbody>
				...
				<tr>
					<td class="w_1">车场类型</td>
					<td class="w_2">
						<ul class="info">
							<li> <input type="checkbox" name="" title="地下停车场" lay-skin="primary" lay-filter="under_ground_carpark"></li>
							<input type="hidden"  name="underGroundCarpark" class="under_ground_carpark">
							
							<li> <input type="checkbox" name="" title="地面停车场" lay-skin="primary" lay-filter="ground_carpark"></li>
							<input type="hidden"  name="groundCarpark" class="ground_carpark">
							
							<li> <input type="checkbox" name="" title="立体停车库" lay-skin="primary" lay-filter="three_Dimensional_carpark"></li>
							<input type="hidden"  name="threeDimensionalCarpark" class="three_Dimensional_carpark">
							
							<li> <input type="checkbox" name="" title="路侧停车场" lay-skin="primary" lay-filter="road_survey_carpark"></li>
							<input type="hidden"  name="roadSurveyCarpark" class="road_survey_carpark">
						</ul>			</td>
				</tr>	
				<tr class="more_option" style="display: table-row;">
					<td class="w_1">车辆类型</td>
					<td class="w_2">
						<ul class="info">
							<li> <input type="checkbox" name="" title="长租卡" lay-skin="primary" lay-filter="long_rent"></li>
							<input type="hidden"  name="longRent" class="long_rent">
							<li> <input type="checkbox" name="" title="临时卡" lay-skin="primary" lay-filter="temporary_card"></li>
							<input type="hidden"  name="temporaryCard" class="temporary_card">
						</ul>			</td>
				</tr>		
				<tr>
  ...
  layui.config({
  ...
  }).use(['index', 'presentCar', 'table','laydate'], function(){
    ...
    form.on('checkbox(under_ground_carpark)', function(data){
		console.log(11)
		if(data.elem.checked){//是否被选中，true或者false
			$('.under_ground_carpark').val(1);
			console.log(data.value);
			console.log(data.elem.checked);
    	} else{
    		$('.under_ground_carpark').val(2);
    	}
	  });
    form.on('checkbox(ground_carpark)', function(data){
		console.log(11)
		if(data.elem.checked){//是否被选中，true或者false
			$('.ground_carpark').val(1);
			console.log(data.value);
			console.log(data.elem.checked);
    	} else{
    		$('.ground_carpark').val(2);
    	}
	  });
    form.on('checkbox(three_Dimensional_carpark)', function(data){
		console.log(11)
		if(data.elem.checked){//是否被选中，true或者false
			$('.three_Dimensional_carpark').val(1);
			console.log(data.value);
			console.log(data.elem.checked);
    	} else{
    		$('.three_Dimensional_carpark').val(2);
    	}
	  });
    form.on('checkbox(road_survey_carpark)', function(data){
		console.log(11)
		if(data.elem.checked){//是否被选中，true或者false
			$('.road_survey_carpark').val(1);
			console.log(data.value);
			console.log(data.elem.checked);
    	} else{
    		$('.road_survey_carpark').val(2);
    	}
	  });
	form.on('checkbox(long_rent)', function(data){
			console.log(11)
			if(data.elem.checked){//是否被选中，true或者false
				$('.long_rent').val(1);
				console.log(data.value);
  ...
  $('.under_ground_carpark').val(2);//初始化复选框值为未选中 
  $('.ground_carpark').val(2);//初始化复选框值为未选中 
  $('.three_Dimensional_carpark').val(2);//初始化复选框值为未选中 
  $('.road_survey_carpark').val(2);//初始化复选框值为未选中 
  
  $('.long_rent').val(2);//初始化复选框值为未选中 
