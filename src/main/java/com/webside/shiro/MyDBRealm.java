...
public class MyDBRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		UserEntity userEntity = plateformAccountMapper.findByName(username);
		if (userEntity != null) {
			if (userEntity.isForbidden()) {
				throw new LockedAccountException(); // 帐号被锁定
			}
			//>>TODO<<
			//the creator of user is locked, then user is also locked!
			//creator should be admin_role SYS_ADMIN, FACTORY_ADM, SUPPLIER_ADM
			//<<TODO>>
